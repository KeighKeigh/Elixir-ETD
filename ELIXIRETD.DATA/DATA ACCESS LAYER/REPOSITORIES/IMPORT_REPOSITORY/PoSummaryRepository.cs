...
namespace ELIXIRETD.DATA.DATA_ACCESS_LAYER.REPOSITORIES.IMPORT_REPOSITORY
{
    public class PoSummaryRepository : IPoSummaryRepository
    {
        ...
        public async Task<bool> ValidateRRAndItemcodeManual(string ponumber,string rrNo, string itemcode, decimal quantity)
        {
            var validate = await _context.PoSummaries.Where(x => x.PO_Number == ponumber && x.RRNo == rrNo)
                                                     .Where(x => x.ItemCode == itemcode)
                                                     .Where(x => x.Delivered == quantity)
                                                     .FirstOrDefaultAsync();
        ...
        public async Task<bool> ValidatePOAndItemcodeManual(string ponumber, string itemcode, decimal quantity)
        {
