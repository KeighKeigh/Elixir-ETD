            ...
            public async Task<Unit> Handle(ConsolidateAuditExportCommand command, CancellationToken cancellationToken)
            {
                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    for (var index = 1; index <= consolidate.Count; index++)
                    {
                        ...
                        if (consolidate[index - 1].TransactionType == "Move Order" || consolidate[index - 1].TransactionType == "Miscellaneous Issue" || consolidate[index - 1].TransactionType == "Borrow" || consolidate[index - 1].TransactionType == "Fuel Register")
                        {
                        ...
                        row.Cell(31).Value = consolidate[index - 1].BusinessUnitCode;
                        row.Cell(32).Value = consolidate[index - 1].BusinessUnitName;
                        row.Cell(33).Value = consolidate[index - 1].DepartmentUnitCode;
