...
namespace ELIXIRETD.DATA.DATA_ACCESS_LAYER.REPOSITORIES.INVENTORY_REPOSITORY
{
    public class MRPInvetoryRepository : IMRPInventory
    {
        ...
        public async Task<PagedList<DtoMRP>> UpdatedMRP(UserParams userParams, string search)
        {
            var EndDate = DateTime.Now;
            var StartDate = EndDate.AddDays(-30);
            string miscReceipt = "MiscellaneousReceipt";
            ...
            if (!string.IsNullOrEmpty(search))
            {
                query = query.Where(x => x.ItemCode.ToLower().Contains(search.ToLower().Trim()) ||
                x.ItemDescription.ToLower().Contains(search.ToLower().Trim()));
            }

            query = query.OrderBy(x => x.ItemCode);

            var getPoSummary = await _context.PoSummaries
            ...
            var getWarehouseIn = await _context.WarehouseReceived
                .AsNoTracking()
                .Where(x => x.IsActive == true && x.TransactionType == receiving)
                .OrderBy(x => x.ActualReceivingDate)
                .GroupBy(x => new
                {
                    x.ItemCode,
            ...
            var getOrderingReserve = await _context.Orders
                  .Where(x => x.IsActive == true && x.PreparedDate != null)
                  .GroupBy(x => new
                  {
                      x.ItemCode,
            ...
            var getNotTransacted = await _context.MoveOrders
               .AsNoTracking()
            ...
            var getReceiptIn = await _context.WarehouseReceived
                .AsNoTracking()
                .Where(x => x.IsActive == true && x.TransactionType == miscReceipt)
                .GroupBy(x => new
                {
                    x.ItemCode,
            ...
            var getIssueOut = await _context.MiscellaneousIssueDetail
                .AsNoTracking()
                .Where(x => x.IsActive == true)
                .GroupBy(x => new
                {
            ...
            var getBorrowedIssue = await _context.BorrowedIssueDetails
                .AsNoTracking()
                .Where(x => x.IsActive == true)
                .GroupBy(x => new
                {
            ...
            var getBorrowedForMrp = await _context.BorrowedIssueDetails
                .AsNoTracking()
                .Where(x => x.IsActive == true)
                .GroupBy(x => new
                {
            ...
            var consumed = await _context.BorrowedConsumes
                .AsNoTracking()
                .Where(x => x.IsActive)
                .GroupBy(x => new
                {
                ...
                {
                    x.Key.ItemCode,
                    x.Key.BorrowedItemPkey,
                    Consume = x.Sum(x => x.Consume != null ? x.Consume : 0)

                }).ToListAsync();


            var returnedBorrowList = await _context.BorrowedIssueDetails
             .Where(x => x.IsActive == true && x.IsReturned == true && x.IsApprovedReturned == true)
             .ToListAsync();

            var getReturnedBorrow = returnedBorrowList
                .Where(x => x.IsActive == true)
                .Where(x => x.IsReturned == true)
                .Where(x => x.IsApprovedReturned == true)
                .GroupJoin(consumed, returned => returned.Id, itemconsume => itemconsume.BorrowedItemPkey, (returned, itemconsume) => new { returned, itemconsume })
                ...
                }).Select(x => new
                {
                    ...
                    x.Key.ItemCode,
                    ReturnQuantity = x.Sum(x => x.returned.Quantity) - x.Sum(x => x.itemconsume?.Consume ?? 0),
                    ConsumeQuantity = x.Sum(x => x.itemconsume?.Consume ?? 0),
                    Quantity = x.Sum(x => x.returned.Quantity)
           ...
           }).Select(fr => new
           {
            ...
            var getSOH = (from warehouse in getWarehouseStock
                          ...
                          select new
                          {
                             ...
                             total.Sum(x => x.returned?.ReturnQuantity ?? 0) -
                             total.Sum(x => x.issue?.Quantity ?? 0) -
                             total.Sum(x => x.borrowed?.Quantity ?? 0) -
                             total.Sum(x => x.moveorder?.QuantityOrdered ?? 0) -
                             total.Sum(x => x.fuel?.Quantity.Value ?? 0)
            ...
            var getReserve = (from warehouse in getWarehouseStock
                              ...
                              select new
                              {
                                  ...
                                  total.Key.ItemCode,
                                  Reserve = total.Sum(x => x.warehouse?.ActualGood ?? 0)
                                  + total.Sum(x => x.returned?.ReturnQuantity ?? 0)
                                  - total.Sum(x => x.ordering?.QuantityOrdered ?? 0)
                                  - total.Sum(x => x.issue?.Quantity ?? 0)
                                  - total.Sum(x => x.borrowed?.Quantity ?? 0) -
                                    total.Sum(x => x.fuel?.Quantity.Value ?? 0)
            ...
            var getSuggestedPo = (from posummary in getPoSummary
                                  ...
                                  select new
                                  {
                                      ...
                                      total.Key.ItemCode,
                                      Ordered = total.Sum(x => x.posummary?.Ordered ?? 0) -
                                                total.Sum(x => x.receive?.ActualGood ?? 0)

                                  });

            var getMiscellaneousIssuePerMonth = await _context.MiscellaneousIssueDetail
                .AsNoTracking()
                .Where(x => x.PreparedDate >= StartDate && x.PreparedDate <= EndDate)
                .Where(x => x.IsActive == true)
                .GroupBy(x => new
            ...
            var returnedBorrowedList = await _context.BorrowedIssueDetails
             .AsNoTracking()
             .Where(x => x.IsActive == true && x.IsApprovedReturned == true && x.IsReturned == true)
             .ToListAsync();

            var borrowedPerMonthList = await _context.BorrowedIssueDetails
                .AsNoTracking()
                .Where(x => x.PreparedDate >= StartDate && x.PreparedDate <= EndDate)
                .Where(x => x.IsActive == true)
                .ToListAsync();
            ...
            var getConsumedPerMonth = returnedBorrowedList
                .GroupJoin(consumed, returned => returned.Id, itemconsume => itemconsume.BorrowedItemPkey, (returned, itemconsume) => new { returned, itemconsume })
                .SelectMany(x => x.itemconsume.DefaultIfEmpty(), (x, itemconsume) => new { x.returned, itemconsume })
                .GroupBy(x => new
                {
            ...
            var getBorrowedOutPerMonth = borrowedPerMonthList
                .GroupJoin(getConsumedPerMonth, returned => returned.ItemCode, itemconsume => itemconsume.ItemCode,
                (returned, itemconsume) => new { returned, itemconsume })
                .SelectMany(x => x.itemconsume.DefaultIfEmpty(), (x, itemconsume) => new { x.returned, itemconsume })
                .GroupBy(x => new
            ...
            var getAvarageIssuance = (from warehouse in getWarehouseStock
                                      ...
                                      select new
                                      {
                                          ...
                                          total.Key.ItemCode,
                                          ActualGood = (total.Sum(x => x.borrowed?.Quantity ?? 0) + total.Sum(x => x.issue?.Quantity ?? 0) + total.Sum(x => x.moveorder?.QuantityOrdered ?? 0)) / 30
            ...
            var getReseverUsage = (from warehouse in getWarehouseStock
                                   ...
                                   {
                                       total.Key.ItemCode,
                                       Reserve = total.Sum(x => x.ordering?.QuantityOrdered ?? 0)

                                   });

            var getWarehouseStockById = await _context.WarehouseReceived
                .AsNoTracking()
                .Select(x => new
                {
                    WarehouseId = x.Id,
            ...
            var getIssueOutId = await _context.MiscellaneousIssueDetail
               .AsNoTracking()
               .Where(x => x.IsActive == true)
               .GroupBy(x => new
               {
            ...
            var getBorrowedIssueId = await _context.BorrowedIssueDetails
                .AsNoTracking()
                .Where(x => x.IsActive == true)
                .GroupBy(x => new
                {
            ...
            var getReturnedBorrowId = returnedBorrowedList
                .GroupJoin(consumed, returned => returned.Id, itemconsume => itemconsume.BorrowedItemPkey, (returned, itemconsume) => new { returned, itemconsume })
                .SelectMany(x => x.itemconsume.DefaultIfEmpty(), (x, itemconsume) => new { x.returned, itemconsume })
                .GroupBy(x => new
                {
                ...
                }).Select(x => new
                {
                    ...
                    x.Key.ItemCode,
                    ReturnQuantity = x.Sum(x => x.returned.Quantity)
                        - x.Sum(x => x.itemconsume?.Consume ?? 0)

                });


            var getfuelRegister = await _context.FuelRegisterDetails
           .AsNoTrackingWithIdentityResolution()
           .Include(x => x.Material)
           .Where(fr => fr.Is_Active == true)
           .GroupBy(fr => new
           ...
           }).Select(fr => new
           {
            ...
            var getUnitPrice = (from warehouse in getWarehouseStockById
                              ...
                              by new
                              {
                                ...
                                select new
                                {
                                    ...
                                    x.Key.WarehouseId,
                                    x.Key.ItemCode,
                                    UnitPrice = x.First().warehouse.UnitPrice * (x.First().warehouse.ActualGood + x.Sum(x => x.returned?.ReturnQuantity ?? 0) - x.Sum(x => x.moveorder?.QuantityOrdered ?? 0) - x.Sum(x => x.issue?.Quantity ?? 0) - x.Sum(x => x.borrow?.Quantity ?? 0)),
                                    ActualGood = x.First().warehouse.ActualGood + (x.Sum(x => x.returned?.ReturnQuantity ?? 0)
                                    - x.Sum(x => x.moveorder?.QuantityOrdered ?? 0) - x.Sum(x => x.issue?.Quantity ?? 0)
                                    - x.Sum(x => x.borrow?.Quantity ?? 0) - x.Sum(x => x.fuel?.Quantity.Value ?? 0))
            ...
            var mrp = (from material in materialList
                       join posummary in getPoSummary
                              on material.ItemCode equals posummary.ItemCode
                              into leftJ1
                       from posummary in leftJ1.DefaultIfEmpty()
                       ...
                       by new
                       {
                           ...
                           sudggest = sudggest?.Ordered ?? 0,
                           warehouseActualGood = warehouse?.ActualGood ?? 0,
                           receiptin = receiptin?.Quantity ?? 0,
                           moveorders = moveorders?.QuantityOrdered ?? 0,
                           issueOut = issueOut?.Quantity ?? 0,
                           borrow = borrow?.Quantity ?? 0,
                           returned = returned?.ReturnQuantity ?? 0,
                           TotalPrice = unitprice?.TotalUnitPrice ?? 0,
                           SOH = SOH?.SOH ?? 0,
                           reserve = reserve?.Reserve ?? 0,
                           averageissuance = averageissuance?.ActualGood ?? 0,
                           usage = usage?.Reserve ?? 0,
                           ConsumeQuantity = consume?.ConsumeQuantity ?? 0,
                           Fuel = fuel?.Quantity ?? 0,
...

                       {
                           ...
                           TotalCost = Math.Round(total.Key.TotalPrice, 2),
                           SOH = total.Key.SOH,
                           PreparedQuantity = total.First().notTransact?.QuantityOrdered ?? 0,
                           Reserve = total.Key.reserve,
                           SuggestedPo = total.Key.sudggest >= 0 ? total.Key.sudggest : 0,
                           AverageIssuance = Math.Round(total.Key.averageissuance, 2),
                           ReserveUsage = total.Key.usage,
            ...
            return PagedList<DtoMRP>.Create(mrp, userParams.PageNumber, userParams.PageSize);
        }
